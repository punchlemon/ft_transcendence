...
### 0.7 Active Session パネル (`/settings/account` 内)
- **目的**: 多端末ログインを可視化し、流出したリフレッシュトークンを UI から即座に失効させる。
- **デザイン**:
  - 2FA セクションの直下にカードを配置し、タイトル「アクティブセッション」「他の端末からのアクセスを管理」。
  - 一覧はテーブルまたはカード表示 (モバイルはカード) で以下の情報を表示:
    1. **デバイス名**: `userAgent` を簡易解析 (例: `Mac • Safari 17`)。解析できない場合は文字列そのまま。
    2. **最終アクティブ**: `lastUsedAt` の相対時間 + 絶対日時ツールチップ。
    3. **場所 / IP**: `ipAddress` の逆引きは行わず、`127.0.0.1` 等の文字列を表示。将来的に GeoIP を足せるようサブテキスト枠を空けておく。
    4. **状態バッジ**: `current` セッションには「現在のデバイス」バッジを付与し、Terminate ボタンを無効化。
    5. **アクション**: その他のセッションには「セッションを終了」ボタン。クリックで確認モーダル → `/auth/sessions/:sessionId` DELETE。
- **データ取得/動作**:
  1. ページ表示時に `GET /auth/sessions` を呼び、レスポンスを `sessionsStore` (もしくはローカル state) に格納。
  2. 成功時は一覧を描画し、`current` が true のレコードは先頭に固定。空配列なら「他の端末でのログインはありません」空状態イラスト。
  3. 「セッションを終了」を実行すると `DELETE /auth/sessions/:id` でリフレッシュトークンを削除 → 成功時はローカル state から該当行を削除し、トーストで通知。
  4. API 失敗時はカード上にインラインエラーを表示 (`SESSION_NOT_FOUND`, `UNAUTHORIZED` 等) し、再取得ボタンを活性化。
  5. 現在端末のセッションは Terminate ボタンを表示せず、代わりに `/auth/logout` を案内。UX 的に「このデバイスからログアウト」CTA を別途配置。
- **テスト観点**:
  - API 成功で `current` バッジが描画され、少なくとも 1 件が現在デバイスとして表示されること。
  - Terminate ボタン押下時に確認モーダル→API→state 更新が順に行われること。
  - DELETE が 404 を返した場合に該当行へ「既に終了しています」と表示して再フェッチ導線が出ること。
  - 空状態およびローディング・エラー UI が切り替わること。
...
- **構成**:
  ...
  2. メインキャンバス: WebGL or Canvas 2D。レスポンシブ比率 16:9。ステータスオーバーレイで `connecting`/`paused`/`finished` を表示。
     - **Game Over**: 勝敗とスコアを大きく表示し、「ロビーへ戻る」ボタンを配置。
     - **Sound**: パドルヒット、壁ヒット、得点、ゲーム終了時に効果音を再生 (Web Audio API)。
...
- **構成**:
  ...
  2. メッセージエリア: 左寄せ (相手)、右寄せ (自分)、システムイベントは中央灰色。ゲーム招待カードは「参加」「拒否」ボタン付き。
     - **DMメニュー**: ヘッダーの「⋮」から「Invite to Game」を選択可能。
